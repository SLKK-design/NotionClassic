...
/* OPT-6: yield callback — called every 16 scalar-mult iterations so
 * MacTCP's .IPP driver can process the TCP SYN-ACK during keygen.   */
static void (*g_yield_hook)(void) = 0;
static unsigned int g_yield_counter = 0;
...
static void EccPoint_mult(uECC_word_t * result,
    ...
    for (i = num_bits - 2; i > 0; --i) {
        ...
        /* OPT-6: yield every 16 iterations so MacTCP processes SYN-ACK */
        if (g_yield_hook && ((++g_yield_counter) & 15u) == 0)
            g_yield_hook();
    }
...
/*
 * uecc_point_multiply - multiply arbitrary point by scalar (P-256 or P-384).
 * curve: uECC_Curve describing the group
 * k, px, py: big-endian byte arrays, curve->num_bytes each
 * rx, ry: output, big-endian byte arrays, curve->num_bytes each
 */
int uecc_point_multiply(uECC_Curve curve,
                        const uint8_t *k,
                        const uint8_t *px, const uint8_t *py,
                        uint8_t *rx, uint8_t *ry)
{
    uECC_word_t point[uECC_MAX_WORDS * 2];
    uECC_word_t scalar[uECC_MAX_WORDS];
    uECC_word_t tmp1[uECC_MAX_WORDS];
    uECC_word_t tmp2[uECC_MAX_WORDS];
    uECC_word_t *p2[2] = {tmp1, tmp2};
    uECC_word_t result[uECC_MAX_WORDS * 2];
    uECC_word_t carry;

    uECC_vli_bytesToNative(point,                    px, curve->num_bytes);
    uECC_vli_bytesToNative(point + curve->num_words, py, curve->num_bytes);
    uECC_vli_bytesToNative(scalar, k, BITS_TO_BYTES(curve->num_n_bits));

    carry = regularize_k(scalar, tmp1, tmp2, curve);
    EccPoint_mult(result, point, p2[!carry], 0, curve->num_n_bits + 1, curve);

    uECC_vli_nativeToBytes(rx, curve->num_bytes, result);
    uECC_vli_nativeToBytes(ry, curve->num_bytes, result + curve->num_words);
    return 1;
}
